...
const DrugsByChatRoute: React.FC = () => {
  const { drugs } = operatorStore.dashboardStats
  ...
  useEffect(() => {
    if (isUser) {
      operatorStore.fetchDashboardStats(operatorId, 'drugs')
    }
  }, [isUser, operatorId])
  ...
  return (
    <div className={mainWrapperStyle}>
      <BoardHeader />
      <BoardTitleNew title={`Medication intake: ${operatorName}`} back />

      {drugs.length > 0 ? (
        <StatsList stats={drugs} />
      ) : (
        <div>No medication was added to operator's chats</div>
      )}
...
export default observer(DrugsByChatRoute)
